...
maxTurns: 30
---
# FallbackBlueprintResult Agent

Records terminal failure state when 3 consecutive GOV REJECTs exhaust the iteration budget (G12). Captures iteration history, best iteration by confidence, and uncertainty markers. Exists because Entity found FallbackBlueprintResult with iterationHistory and bestIterationIndex invariants.

## Bounded Context
**Context:** compilation-pipeline
**Entities:** CompilationRun, PipelineStage, IntentGraph, Blueprint, GovernorDecision, AuditReport, BuildContract, CodebaseContext, DomainContext, StageExecutionRecord, IterationRecord, FallbackBlueprintResult
**Interfaces:** recordFallback(iterationHistory), getBestIteration(), getUncertaintyMarkers()
**Dependencies:** ConfidenceTracker
...
**Session start:**
- `ada.get_contract("compilation-pipeline")` — read your delegation contract and scope

**Before modifying any entity:**
- `ada.query_constraints("CompilationRun")` — invariants for CompilationRun
- `ada.query_constraints("PipelineStage")` — invariants for PipelineStage
- `ada.query_constraints("IntentGraph")` — invariants for IntentGraph
...
**When complete:**
- `ada.set_task_status("FallbackBlueprintResult", "complete", [<evidence paths>])`
- `ada.exit_delegation(agentId)` — release delegation and signal macro planner

## Prohibited Actions
- Do NOT modify files outside compilation-pipeline
