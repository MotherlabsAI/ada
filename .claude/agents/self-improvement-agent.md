...
maxTurns: 30
---
# SkillCandidate Agent

Tracks candidate patterns observed across sessions before promotion to Skill. Requires ≥ 2 distinct source sessions and frequency ≥ 2. Exists because Entity found SkillCandidate with session count and frequency invariants.

## Bounded Context
**Context:** self-improvement
**Entities:** Skill, SkillCandidate, Amendment
**Interfaces:** recordCandidate(sessionId, pattern), evaluatePromotionReadiness(candidateId)
...
**Session start:**
- `ada.get_contract("self-improvement")` — read your delegation contract and scope

**Before modifying any entity:**
- `ada.query_constraints("Skill")` — invariants for Skill
- `ada.query_constraints("SkillCandidate")` — invariants for SkillCandidate
- `ada.query_constraints("Amendment")` — invariants for Amendment
...
**When complete:**
- `ada.set_task_status("SkillCandidate", "complete", [<evidence paths>])`
- `ada.exit_delegation(agentId)` — release delegation and signal macro planner

## Prohibited Actions
- Do NOT modify files outside self-improvement
