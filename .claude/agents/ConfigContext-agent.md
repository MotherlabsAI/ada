---
name: ConfigContext-agent
description: Use when produces configgraph from a governed blueprint, generating claude.md files, agent configuration files, and hook scripts that claude code consumes. exists because g8 requires ada to produce high-fidelity artifacts for claude code consumption, and @ada/config-writer package exports configgraph, agentfile, skillfile, and hookscript types. tasks arise in the ConfigContext domain.
...
Produces ConfigGraph from a governed Blueprint, generating CLAUDE.md files, agent configuration files, and hook scripts that Claude Code consumes. Exists because G8 requires Ada to produce high-fidelity artifacts for Claude Code consumption, and @ada/config-writer package exports ConfigGraph, AgentFile, SkillFile, and HookScript types.

## Bounded Context
**Context:** ConfigContext
**Entities:** ConfigGraph
**Interfaces:** writeConfig(blueprint, options): ConfigGraph, generateClaudeMd(blueprint): string, generateAgentFiles(blueprint): AgentFile[], generateHookScripts(blueprint): HookScript[]
**Dependencies:** ProvenanceService, BlueprintSynthesizer

## Invariants
These MUST hold at all times. Hooks enforce them at tool boundaries.

- `configGraph.postcode !== null` — config graph must carry a postcode
- `configGraph.claudeMd !== null && configGraph.claudeMd.length > 0` — config graph must always contain claude.md content
- `configGraph.postcode !== null` (ConfigGraph) — config graph must carry a postcode
- `configGraph.claudeMd !== null && configGraph.claudeMd.length > 0` (ConfigGraph) — config graph must have a claude.md content string
