...
Implements the ambiguity-resolution workflow. Evaluates blocking IntentUnknowns, generates ClarificationRequests, collects user responses via CLI, applies defaults for non-blocking unknowns, and merges resolutions back into the IntentGraph. Drives the IntentUnknown state machine (DETECTED→PENDING_CLARIFICATION→RESOLVED_BY_USER|RESOLVED_BY_DEFAULT|UNRESOLVABLE).

## Bounded Context
**Context:** Intent
**Entities:** IntentGraph, IntentGoal, IntentUnknown, ClarificationRequest
...
- `intentContext.blockingUnknowns.every(u => clarificationAnswers.some(a => a.unknownId === u.id))` — all blocking unknowns must have a corresponding ClarificationAnswer before the pipeline may begin
- `intentGraph.rawIntent !== null && intentGraph.rawIntent.trim().length > 0` (IntentGraph) — raw intent must be non-empty — no empty-string compilations
- `intentGraph.goals.length > 0` (IntentGraph) — every intent must yield at least one goal
- `intentGraph.postcode.stage === 'INT'` (IntentGraph) — postcode stage must match INT stage
- `intentGraph.unknowns.filter(u => u.impact === 'blocking').length === 0 || clarificationsResolved === true` (IntentGraph) — blocking unknowns must be resolved before pipeline execution
- `intentGoal.id !== null && intentGoal.id.length > 0` (IntentGoal) — goal must have a stable identifier for provenance tracing
- `intentGoal.description !== null && intentGoal.description.trim().length > 0` (IntentGoal) — goal description must not be blank
- `intentUnknown.id !== null && intentUnknown.id.length > 0` (IntentUnknown) — unknown must have a stable id for clarification linkage
- `intentUnknown.impact !== null` (IntentUnknown) — impact classification must be present to determine pipeline gate behaviour
- `clarificationRequest.unknownId !== null && clarificationRequest.unknownId.length > 0` (ClarificationRequest) — must reference an existing IntentUnknown by id
- `clarificationRequest.question !== null && clarificationRequest.question.trim().length > 0` (ClarificationRequest) — question text must be non-empty before surfacing to user
- `clarificationRequest.impact === 'blocking' ? clarificationRequest.suggestedDefault === null : true` (ClarificationRequest) — blocking clarifications must not carry a suggested default — they require explicit user resolution
