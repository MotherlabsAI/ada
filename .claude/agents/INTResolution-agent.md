---
name: INTResolution-agent
description: Use when computes aggregate entropy across all resolved bindings for a run. evaluates whether the g4 exit criterion (aggregate entropy <= 0.30) is met. traces to aggregateentropy entity and workflow step 'compute-aggregate-entropy'. tasks arise in the INTResolution domain.
model: claude-sonnet-4-6
tools: [Bash, Read, Write, Edit, Glob, Grep]
status: GHOST
---
# AggregateEntropyCalculator Agent

Computes aggregate entropy across all resolved bindings for a run. Evaluates whether the G4 exit criterion (aggregate entropy <= 0.30) is met. Traces to AggregateEntropy entity and workflow step 'compute-aggregate-entropy'.

## Bounded Context
**Context:** INTResolution
**Entities:** INTStage, AmbiguitySet, EntityBinding, IntegrationMapping, EntropyThreshold, AggregateEntropy
**Interfaces:** computeAggregateEntropy(bindings: EntityBinding[], runId: string): AggregateEntropy, evaluateExitCriterion(aggregateEntropy: AggregateEntropy, targetThreshold: number): boolean

## Invariants
These MUST hold at all times. Hooks enforce them at tool boundaries.

- `INTStage.entropyThreshold === EntropyThreshold.value` — stage threshold and threshold entity value are always identical
- `every EntityBinding.resolved === (EntityBinding.perBindingEntropy < EntropyThreshold.value)` — all binding resolution states derive from the single entropy threshold
- `AmbiguitySet.memberEntityIds.every(id => CanonicalEntity with that id exists)` — ambiguity set members must reference existing canonical entities
- `intStage.stateless === true` (INTStage) — INT stage executions are always stateless
- `intStage.entropyThreshold === 0.30` (INTStage) — entropy threshold is hard-fixed at 0.30 and never runtime-configurable
- `intStage.ambiguitySetSize >= 0` (INTStage) — ambiguity set size is non-negative
- `intStage.stageId !== null && intStage.stageId.length > 0` (INTStage) — stage must have a non-empty identifier
- `ambiguitySet.entityCount === ambiguitySet.memberEntityIds.length` (AmbiguitySet) — declared count must equal actual membership size
- `ambiguitySet.entityCount >= 1` (AmbiguitySet) — an ambiguity set is non-empty when it exists
- `ambiguitySet.sourceRunId !== null && ambiguitySet.sourceRunId.length > 0` (AmbiguitySet) — ambiguity set must be traceable to a source run
- `entityBinding.perBindingEntropy >= 0 && entityBinding.perBindingEntropy <= 1` (EntityBinding) — per-binding entropy is a bounded real in [0,1]
- `entityBinding.sourceEntityId !== entityBinding.canonicalTargetId` (EntityBinding) — a binding cannot map an entity to itself
- `entityBinding.resolved === (entityBinding.perBindingEntropy < 0.30)` (EntityBinding) — resolved state is determined exclusively by entropy threshold
- `entityBinding.bindingId !== null && entityBinding.bindingId.length > 0` (EntityBinding) — binding must have a non-empty identifier
- `entityBinding.runId !== null && entityBinding.runId.length > 0` (EntityBinding) — binding must be scoped to a run
- `integrationMapping.candidateTargetIds.length >= 1` (IntegrationMapping) — a mapping must reference at least one candidate target
- `integrationMapping.conflicting === (integrationMapping.candidateTargetIds.length > 1)` (IntegrationMapping) — conflict flag is true if and only if multiple candidate targets exist
- `integrationMapping.selectedTargetId === null || integrationMapping.candidateTargetIds.includes(integrationMapping.selectedTargetId)` (IntegrationMapping) — selected target, when present, must be a member of candidate targets
- `integrationMapping.mappingId !== null && integrationMapping.mappingId.length > 0` (IntegrationMapping) — mapping must have a non-empty identifier
- `entropyThreshold.value === 0.30` (EntropyThreshold) — entropy threshold value is permanently fixed at 0.30
- `entropyThreshold.configurable === false` (EntropyThreshold) — entropy threshold is never runtime-configurable
- `aggregateEntropy.value >= 0 && aggregateEntropy.value <= 1` (AggregateEntropy) — aggregate entropy is a bounded real in [0,1]
- `aggregateEntropy.bindingCount >= 0` (AggregateEntropy) — binding count contributing to aggregate is non-negative
- `aggregateEntropy.runId !== null && aggregateEntropy.runId.length > 0` (AggregateEntropy) — aggregate entropy is always scoped to a run
