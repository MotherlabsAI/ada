---
name: OrchestratorContext-agent
description: Use when produces configuration artifacts from compilation outputs. writes agentfile definitions (name, model, path), skillfile definitions, hookscript definitions, and assembled configgraph (claudemd, postcode, agents, skills). consumes blueprint and blueprintcomponents to derive agent and skill configurations. manages writeconfigoptions for output customization. tasks arise in the OrchestratorContext domain.
...
Produces configuration artifacts from compilation outputs. Writes AgentFile definitions (name, model, path), SkillFile definitions, HookScript definitions, and assembled ConfigGraph (claudeMd, postcode, agents, skills). Consumes Blueprint and BlueprintComponents to derive agent and skill configurations. Manages WriteConfigOptions for output customization.

## Bounded Context
**Context:** OrchestratorContext
**Entities:** SessionCheckpoint, SpawnConfig, ClaudeEvent, ConfigGraph, AgentFile, SkillFile, HookScript
**Interfaces:** writeAgentFile(agentFile), writeSkillFile(skillFile), writeHookScript(hookScript), assembleConfigGraph(blueprint, options), emitConfig(configGraph, writeOptions)
**Dependencies:** ProvenanceService

## Invariants
These MUST hold at all times. Hooks enforce them at tool boundaries.

- `sessionCheckpoint.blueprint !== null` — Checkpoint must carry a blueprint — a checkpoint without a blueprint cannot recover pipeline state
- `sessionCheckpoint.sessionId !== null && sessionCheckpoint.sessionId.length > 0` (SessionCheckpoint) — Checkpoint must reference a session — a checkpoint without a session cannot be restored by the orchestrator
- `sessionCheckpoint.blueprint !== null` (SessionCheckpoint) — Checkpoint must carry a blueprint — without the blueprint state the checkpoint has no recovery value
- `sessionCheckpoint.iterationCount >= 0` (SessionCheckpoint) — Iteration count must be non-negative — negative counts are structurally invalid
- `sessionCheckpoint.timestamp > 0` (SessionCheckpoint) — Checkpoint must be timestamped — unordered checkpoints cannot be used to determine the latest recoverable state
- `configGraph.claudeMd !== null && configGraph.claudeMd.length > 0` (ConfigGraph) — Config graph must carry a root configuration document — without it the agent configuration is incomplete
- `configGraph.postcode !== null` (ConfigGraph) — ConfigGraph must carry a postcode — it is a typed output artifact requiring provenance
- `configGraph.agents.length > 0 || configGraph.skills.length > 0` (ConfigGraph) — Config graph must define at least one agent or skill — an empty config graph configures nothing
- `agentFile.name !== null && agentFile.name.length > 0` (AgentFile) — Agent must be named — anonymous agents cannot be referenced in the config graph
- `agentFile.model !== null && agentFile.model.length > 0` (AgentFile) — Agent must declare a model — a modelless agent cannot execute
- `agentFile.path !== null && agentFile.path.length > 0` (AgentFile) — Agent must have a file path — pathless agents cannot be written to the config output
