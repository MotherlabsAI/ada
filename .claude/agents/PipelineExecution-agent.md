...
description: Use when PipelineExecution tasks arise. Owns PipelineOrchestrator. Does not modify files outside PipelineExecution.
...
Coordinates the 7-stage ada-semantic-compilation-pipeline workflow: initializes CompilationRun, sequences stage execution, manages PipelineState transitions (idle→loading→binding→extracting→validating→gating→emitting→success/failed), enforces ProvenanceGate entropy checks between stages, and emits the final CompileResult with GovernorDecision.

## Bounded Context
**Context:** PipelineExecution
**Entities:** CompileResult, PipelineState, CompilationRun, StageExecutionRecord, GovernorDecision, ProvenanceGate, PostcodeAddress
**Interfaces:** initializeCompilationRun(sourceIntent: RawIntent): CompilationRun, executeStage(stageCode: CompilerStageCode, input: PipelineState): StageExecutionRecord, checkProvenanceGate(from: PostcodeAddress, to: PostcodeAddress): ProvenanceGate, transitionState(current: PipelineState, event: StageCompleteEvent): PipelineState, requestGovernorDecision(state: PipelineState): GovernorDecision, emitCompileResult(run: CompilationRun, state: PipelineState, decision: GovernorDecision): CompileResult
**Dependencies:** BlueprintRegistryLoader, C3GapCollapseResolver, EntityExtractor, ProvenanceChainValidator, ENTGateEvaluator
...
**Session start:**
- `ada.get_contract("PipelineExecution")` — read your delegation contract and scope

**Before modifying any entity:**
- `ada.query_constraints("CompileResult")` — invariants for CompileResult
- `ada.query_constraints("PipelineState")` — invariants for PipelineState
- `ada.query_constraints("CompilationRun")` — invariants for CompilationRun
...
**When complete:**
- `ada.set_task_status("PipelineOrchestrator", "complete", [<evidence paths>])`
- `ada.exit_delegation(agentId)` — release delegation and signal macro planner

## Prohibited Actions
- Do NOT modify files outside PipelineExecution
