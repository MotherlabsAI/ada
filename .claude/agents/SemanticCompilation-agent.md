...
- `CompilationRun.stagesExecuted must include all four stage names in order: intent_parse, semantic_analysis, blueprint_emission, governance_validation` — all four stages must execute in declared order
- `CompilationRun.determinismMetadata.temperature >= 0` — temperature must be non-negative across all runs
- `compilerStage.stageId !== null` (CompilerStage) — stage must be identified
- `compilerStage.name !== null` (CompilerStage) — stage must have a declared name
- `compilerStage.inputSchema !== null` (CompilerStage) — stage must declare its input schema
- `compilerStage.outputSchema !== null` (CompilerStage) — stage must declare its output schema
- `compilerStage.pinnedPromptRef !== null` (CompilerStage) — every stage must reference a pinned prompt
- `determinismMetadata.modelId !== null && determinismMetadata.modelId.length > 0` (DeterminismMetadata) — model identifier must be recorded
- `determinismMetadata.temperature >= 0 && determinismMetadata.temperature <= 1` (DeterminismMetadata) — temperature must be in valid range
- `determinismMetadata.promptRegistryVersion !== null` (DeterminismMetadata) — prompt registry version used must be recorded
- `determinismMetadata.structuredOutputEnforced !== null` (DeterminismMetadata) — structured output enforcement flag must be set
- `compilationRun.runId !== null` (CompilationRun) — run must be identified
- `compilationRun.sourceIntent !== null` (CompilationRun) — every run must have a source intent
- `compilationRun.stagesExecuted !== null` (CompilationRun) — stages executed list must be initialized
- `(compilationRun.resultBlueprintId !== null) || (compilationRun.fallbackResultId !== null)` (CompilationRun) — a completed run must produce either a blueprint or a fallback result
- `compilationRun.startedAt !== null` (CompilationRun) — run start time must be recorded
- `compilationRun.determinismMetadata !== null` (CompilationRun) — determinism metadata must be captured per run
