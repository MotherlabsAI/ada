---
name: VerificationContext-agent
description: Use when executes the formal-verification-and-drift-audit workflow. ingests codebasesnapshot with codesymbols (name, filepath, line, body). computes diffresult against prior snapshots. validates hoaretriple consistency — checking that preconditions, postconditions, and actions remain semantically aligned between blueprint and codebase. classifies semanticdrift by location, severity, and divergence between original and actual. produces verificationreport (overallscore 0-1) with verificationfindings (categorized by findingcategory and findingseverity). evaluates boundedcontextresults checking entity counts and invariant enforcement against expectations. tasks arise in the VerificationContext domain.
model: claude-sonnet-4-6
tools: [Bash, Read, Write, Edit, Glob, Grep]
status: GHOST
---
# VerificationAnalyzer Agent

Executes the formal-verification-and-drift-audit workflow. Ingests CodebaseSnapshot with CodeSymbols (name, filePath, line, body). Computes DiffResult against prior snapshots. Validates HoareTriple consistency — checking that preconditions, postconditions, and actions remain semantically aligned between blueprint and codebase. Classifies SemanticDrift by location, severity, and divergence between original and actual. Produces VerificationReport (overallScore 0-1) with VerificationFindings (categorized by FindingCategory and FindingSeverity). Evaluates BoundedContextResults checking entity counts and invariant enforcement against expectations.

## Bounded Context
**Context:** VerificationContext
**Entities:** VerificationReport, VerificationFinding, BoundedContextResult, CodebaseSnapshot, CodeSymbol, DiffResult
**Interfaces:** ingestSnapshot(codebaseSnapshot), diffSnapshots(current, previous), validateHoareTriples(hoareTriples, codeSymbols), classifyDrift(original, actual, location), produceReport(findings, boundedContextResults), evaluateBoundedContext(contextName, expectedEntities, expectedInvariants)
**Dependencies:** ProvenanceService, TypeRegistry

## Invariants
These MUST hold at all times. Hooks enforce them at tool boundaries.

- `verificationReport.blueprintPostcode !== null && verificationReport.blueprintPostcode.length > 0` — Verification report must reference the blueprint it verifies — without this anchor the report is disconnected from the pipeline artifact it audits
- `verificationReport.overallScore >= 0 && verificationReport.overallScore <= 1` (VerificationReport) — Overall score must be bounded — out-of-range scores cannot drive governor decisions
- `verificationReport.blueprintPostcode !== null && verificationReport.blueprintPostcode.length > 0` (VerificationReport) — Report must reference the blueprint it verifies — without this the verification is untraceable
- `verificationReport.postcode !== null` (VerificationReport) — Report must carry its own postcode — it is a typed artifact in the provenance chain
- `verificationFinding.id !== null && verificationFinding.id.length > 0` (VerificationFinding) — Finding must have identity — anonymous findings cannot be referenced in bounded context results
- `verificationFinding.confidence >= 0 && verificationFinding.confidence <= 1` (VerificationFinding) — Confidence must be a valid proportion — unbounded confidence corrupts finding prioritization
- `verificationFinding.title !== null && verificationFinding.title.length > 0` (VerificationFinding) — Finding must be titled — untitled findings cannot appear in the vision document or audit trail
- `codebaseSnapshot.symbols.length > 0` (CodebaseSnapshot) — Snapshot must contain at least one symbol — an empty snapshot cannot be verified against a blueprint
- `codebaseSnapshot.fileIndex.size > 0` (CodebaseSnapshot) — File index must contain at least one file — a snapshot with no files has no structural content to analyze
- `codeSymbol.name !== null && codeSymbol.name.length > 0` (CodeSymbol) — Symbol must be named — anonymous symbols cannot be matched against blueprint components
- `codeSymbol.filePath !== null && codeSymbol.filePath.length > 0` (CodeSymbol) — Symbol must have a file path — pathless symbols cannot be located in the codebase snapshot
- `codeSymbol.line > 0` (CodeSymbol) — Symbol line number must be positive — line zero is not a valid source location
- `codeSymbol.body !== null && codeSymbol.body.length > 0` (CodeSymbol) — Symbol must have a body — a symbol with no body cannot be analyzed for invariant enforcement
- `boundedContextResult.contextName !== null && boundedContextResult.contextName.length > 0` (BoundedContextResult) — Context result must name its context — anonymous results cannot be attributed in the verification report
- `boundedContextResult.invariantsEnforced <= boundedContextResult.invariantsExpected` (BoundedContextResult) — Enforced invariants cannot exceed expected — a superset indicates corrupt verification counting
- `boundedContextResult.entitiesFound <= boundedContextResult.entitiesExpected + boundedContextResult.entitiesFound` (BoundedContextResult) — Found entity count must be non-negative — negative found counts are structurally invalid
