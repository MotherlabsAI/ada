---
name: TypeScriptProjectGraph-agent
description: Use when validates that typescript project configurations are consistent with the workspace dependency graph: each package's tsconfig has a valid outdir, composite mode is enabled where project references are used, and project references match workspace protocol dependencies. ensures roottsconfig references all package tsconfigs. tasks arise in the TypeScriptProjectGraph domain.
model: claude-sonnet-4-6
tools: [Bash, Read, Write, Edit, Glob, Grep]
status: GHOST
---
# TsConfigValidator Agent

Validates that TypeScript project configurations are consistent with the workspace dependency graph: each package's tsconfig has a valid outDir, composite mode is enabled where project references are used, and project references match workspace protocol dependencies. Ensures RootTsConfig references all package tsconfigs.

## Bounded Context
**Context:** TypeScriptProjectGraph
**Entities:** RootTsConfig, TsConfig, ProjectReference, TopologicalBuildOrder
...
- `rootTsConfig.referencedPackageTsConfigs.length === pnpmWorkspace.packages.length` — root tsconfig must reference a tsconfig for every workspace package
- `topologicalBuildOrder.cycleDetected === false` — the resolved project reference graph must be acyclic
- `tsConfig.path !== null && tsConfig.path.endsWith('.json')` (TsConfig) — tsconfig must be a JSON file
- `tsConfig.composite === true || tsConfig.projectReferences.length === 0` (TsConfig) — a tsconfig with project references must have composite enabled
- `tsConfig.outDir !== null && tsConfig.outDir.length > 0` (TsConfig) — tsconfig must declare an output directory
- `tsConfig.ownerPackage !== null` (TsConfig) — tsconfig must be owned by a named package
- `projectReference.sourcePackage !== projectReference.referencedPackage` (ProjectReference) — a package cannot reference itself
- `projectReference.referencedTsConfigPath !== null && projectReference.referencedTsConfigPath.length > 0` (ProjectReference) — reference must point to a concrete tsconfig path
- `topologicalBuildOrder.cycleDetected === false` (TopologicalBuildOrder) — topological order must contain no cycles — a cycle makes build order undefined
- `topologicalBuildOrder.orderedPackages.length > 0` (TopologicalBuildOrder) — at least one package must be present in the ordered list
- `topologicalBuildOrder.orderedPackages.length === new Set(topologicalBuildOrder.orderedPackages).size` (TopologicalBuildOrder) — each package must appear exactly once in the ordered list
- `rootTsConfig.path !== null && rootTsConfig.path.endsWith('.json')` (RootTsConfig) — root tsconfig must be a JSON file
- `rootTsConfig.composite === true` (RootTsConfig) — root tsconfig must enable composite for project-wide build orchestration
- `rootTsConfig.referencedPackageTsConfigs.length > 0` (RootTsConfig) — root tsconfig must reference at least one package tsconfig
