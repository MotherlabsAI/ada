...
description: Use when generates agent configuration files (agentfile, skillfile, hookscript) from blueprint specifications. maps to @ada/config-writer package. translates blueprint architecture into claude code agent configuration graph. tasks arise in the Governance domain.
...
Generates agent configuration files (AgentFile, SkillFile, HookScript) from Blueprint specifications. Maps to @ada/config-writer package. Translates Blueprint architecture into Claude Code agent configuration graph.

## Bounded Context
**Context:** Governance
**Entities:** Blueprint, AuditReport, GovernorDecision, PolicyViolation
...
- `governanceContext.blueprint !== null ? governanceContext.blueprint.postcode !== null : true` — a Blueprint may not be emitted without a postcode — postcode is the provenance anchor
- `governanceContext.governorDecision.decision === 'ACCEPT' ? governanceContext.auditReport.passed === true : true` — Governor ACCEPT requires a passing AuditReport
- `blueprint.summary !== null && blueprint.summary.trim().length > 0` (Blueprint) — blueprint must carry a non-empty summary
- `blueprint.dataModel !== null` (Blueprint) — blueprint must embed a data model — a blueprint without entities is structurally incomplete
- `blueprint.processModel !== null` (Blueprint) — blueprint must embed a process model — a blueprint without processes is behaviourally incomplete
- `blueprint.postcode.stage === 'SYN'` (Blueprint) — blueprint postcode must carry the SYN stage tag
- `blueprint.architecture.components.length > 0` (Blueprint) — blueprint architecture must define at least one component
- `auditReport.coverageScore >= 0 && auditReport.coverageScore <= 1` (AuditReport) — coverage score must be a ratio in [0,1]
- `auditReport.coherenceScore >= 0 && auditReport.coherenceScore <= 1` (AuditReport) — coherence score must be a ratio in [0,1]
- `auditReport.passed === true ? auditReport.drifts.filter(d => d.severity === 'critical').length === 0 : true` (AuditReport) — a passed audit must have zero critical drifts
- `auditReport.postcode.stage === 'AUD'` (AuditReport) — audit report postcode must carry the AUD stage tag
- `governorDecision.confidence >= 0 && governorDecision.confidence <= 1` (GovernorDecision) — confidence must be a probability in [0,1]
- `governorDecision.decision === 'REJECT' ? governorDecision.rejectionReasons.length > 0 : true` (GovernorDecision) — a REJECT decision must carry at least one rejection reason
- `governorDecision.decision === 'ACCEPT' ? governorDecision.provenanceIntact === true : true` (GovernorDecision) — Governor may only ACCEPT when provenance chain is intact
- `governorDecision.gatePassRate >= 0 && governorDecision.gatePassRate <= 1` (GovernorDecision) — gate pass rate must be in [0,1]
- `governorDecision.postcode.stage === 'GOV'` (GovernorDecision) — governor decision postcode must carry the GOV stage tag
- `policyViolation.ruleViolated !== null && policyViolation.ruleViolated.trim().length > 0` (PolicyViolation) — violation must name the rule that was broken
- `policyViolation.description !== null && policyViolation.description.trim().length > 0` (PolicyViolation) — violation description must be non-empty
