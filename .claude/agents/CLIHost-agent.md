---
name: CLIHost-agent
description: Use when infrastructure adapter that wraps the anthropic api client. accepts a pinnedprompt template, intent context, and determinismmetadata parameters (temperature, modelid), makes the api call with structured output enforcement, and returns the typed response. centralizes all llm interaction to enforce c2 (anthropic only), c4 (determinism controls), and api key usage. every component that needs llm inference calls this adapter rather than the anthropic sdk directly. tasks arise in the CLIHost domain.
model: claude-sonnet-4-6
tools: [Bash, Read, Write, Edit, Glob, Grep]
status: GHOST
---
# AnthropicInferenceAdapter Agent
...
## Bounded Context
**Context:** CLIHost
**Entities:** AdaCLI, APIKeyConfiguration, PromptRegistry, PinnedPrompt
...
- `every PinnedPrompt in PromptRegistry.entries must have a unique stageTarget+version combination` — no two prompts in registry share the same stage and version
- `AdaCLI.activePromptRegistry.entries covers all four CompilerStage names` — CLI must have a prompt registered for every stage
- `adaCLI.name === 'ada'` (AdaCLI) — tool name is fixed as 'ada'
- `adaCLI.license === 'MIT'` (AdaCLI) — project must be MIT licensed
- `adaCLI.apiKeyConfig !== null` (AdaCLI) — API key configuration must be present
- `adaCLI.activePromptRegistry !== null` (AdaCLI) — prompt registry must be bound at startup
- `apiKeyConfiguration.keyValue !== null && apiKeyConfiguration.keyValue.length > 0` (APIKeyConfiguration) — key value must be non-empty
- `apiKeyConfiguration.source !== null` (APIKeyConfiguration) — key source must be declared
- `apiKeyConfiguration.provider !== null` (APIKeyConfiguration) — provider must be identified
- `pinnedPrompt.promptId !== null` (PinnedPrompt) — prompt must be identified
- `pinnedPrompt.version !== null && pinnedPrompt.version.length > 0` (PinnedPrompt) — prompt version must be non-empty
- `pinnedPrompt.templateText !== null && pinnedPrompt.templateText.length > 0` (PinnedPrompt) — prompt must have template text
- `pinnedPrompt.stageTarget !== null` (PinnedPrompt) — prompt must declare which stage it targets
- `pinnedPrompt.structuredOutputSchema !== null` (PinnedPrompt) — prompt must declare its expected output schema
- `promptRegistry.registryId !== null` (PromptRegistry) — registry must be identified
- `promptRegistry.entries !== null && promptRegistry.entries.length > 0` (PromptRegistry) — registry must contain at least one prompt
- `promptRegistry.activeVersion !== null` (PromptRegistry) — registry must declare an active version
