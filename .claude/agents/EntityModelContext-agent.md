---
name: EntityModelContext-agent
description: Use when builds the entitymap from domaincontext and intentgraph, identifying entities with their categories (substance/quality/relation/event/state), properties, invariants, and bounded contexts. maps to the ent-build-entity-model step. exists because entity and entitymap are the structural backbone of the blueprint's data model, and boundedcontext groupings determine component boundaries downstream. tasks arise in the EntityModelContext domain.
...
Builds the EntityMap from DomainContext and IntentGraph, identifying entities with their categories (substance/quality/relation/event/state), properties, invariants, and bounded contexts. Maps to the ENT-build-entity-model step. Exists because Entity and EntityMap are the structural backbone of the blueprint's data model, and BoundedContext groupings determine component boundaries downstream.

## Bounded Context
**Context:** EntityModelContext
**Entities:** EntityMap, Entity, EntityInvariant, BoundedContext, BoundedContextResult
**Interfaces:** buildEntityMap(domainContext, intentGraph, callbacks): EntityMap, deriveEntities(domainContext): Entity[], extractInvariants(entity): EntityInvariant[], assignBoundedContexts(entities): BoundedContext[], validateEntityMap(entityMap): Challenge[]
**Dependencies:** ProvenanceService, DomainAnalyzer

## Invariants
These MUST hold at all times. Hooks enforce them at tool boundaries.

- `boundedContext.entities.includes(boundedContext.rootEntity)` — every bounded context root entity must be a member of that context
- `entity.invariants.length > 0` — every entity must carry at least one invariant
- `entityMap.entities.length > 0` (EntityMap) — entity map must contain at least one entity
- `entityMap.boundedContexts.length > 0` (EntityMap) — at least one bounded context must be defined
- `entityMap.postcode !== null` (EntityMap) — entity map must carry a postcode
- `entity.name !== null && entity.name.length > 0` (Entity) — entity must have a non-empty name
- `entity.properties.length > 0` (Entity) — entity must have at least one property
- `entity.invariants.length > 0` (Entity) — entity must have at least one invariant
- `['substance','quality','relation','event','state'].includes(entity.category)` (Entity) — entity category must be one of the five defined ontological values
- `boundedContext.name !== null && boundedContext.name.length > 0` (BoundedContext) — bounded context must have a name
- `boundedContext.rootEntity !== null && boundedContext.rootEntity.length > 0` (BoundedContext) — bounded context must declare a root entity
- `boundedContext.entities.includes(boundedContext.rootEntity)` (BoundedContext) — root entity must be a member of the bounded context's entity list
- `entityInvariant.predicate !== null && entityInvariant.predicate.length > 0` (EntityInvariant) — invariant must have a non-empty predicate expression
- `entityInvariant.description !== null && entityInvariant.description.length > 0` (EntityInvariant) — invariant must have a human-readable description
- `boundedContextResult.contextName !== null && boundedContextResult.contextName.length > 0` (BoundedContextResult) — result must identify its bounded context
- `boundedContextResult.entitiesFound <= boundedContextResult.entitiesExpected || boundedContextResult.entitiesExpected === 0` (BoundedContextResult) — entities found should not exceed expected unless context is unconstrained
- `boundedContextResult.invariantsEnforced <= boundedContextResult.invariantsExpected || boundedContextResult.invariantsExpected === 0` (BoundedContextResult) — enforced invariants must not exceed the number expected
