---
name: DialogueExchangeContext-agent
description: Use when owns the elicitationturn aggregate root and manages the dialogue exchange loop. generates clarificationrequests from gaps (using existing clarificationrequest type from @ada/compiler per c4), generates adaproposals with rationale for structural additions, receives clarificationanswers and proposal dispositions from the user, and manages turn state machine (opened → awaiting_answer → answered → closed | expired). this is where ada's active participation (g3) is realized — she surfaces gaps as targeted questions and proposes structure rather than passively validating. tasks arise in the DialogueExchangeContext domain.
...
Owns the ElicitationTurn aggregate root and manages the dialogue exchange loop. Generates ClarificationRequests from Gaps (using existing ClarificationRequest type from @ada/compiler per C4), generates AdaProposals with rationale for structural additions, receives ClarificationAnswers and proposal dispositions from the user, and manages turn state machine (opened → awaiting_answer → answered → closed | expired). This is where Ada's active participation (G3) is realized — she surfaces gaps as targeted questions and proposes structure rather than passively validating.

## Bounded Context
**Context:** DialogueExchangeContext
**Entities:** ElicitationTurn, ClarificationRequestRecord, ClarificationAnswerRecord, AdaProposal
**Interfaces:** openTurn(sessionId: string, gap: Gap): ElicitationTurn, generateClarificationRequest(gap: Gap): ClarificationRequestRecord, generateAdaProposal(gap: Gap, draftContext: DraftIntentGraph): AdaProposal, receiveClarificationAnswer(turnId: string, answer: string): ClarificationAnswerRecord, processProposalDisposition(proposalId: string, disposition: string, modifiedText?: string): AdaProposal, closeTurn(turnId: string): ElicitationTurn, expireTurn(turnId: string): ElicitationTurn
**Dependencies:** GapAnalyzer, DraftIntentGraphManager

## Invariants
These MUST hold at all times. Hooks enforce them at tool boundaries.

- `elicitationTurn.clarificationRequestId !== null && clarificationRequestRecord.clarificationRequestId === elicitationTurn.clarificationRequestId` — Every turn must reference a stored ClarificationRequestRecord — without this, the question being answered cannot be retrieved
- `elicitationTurn.status === 'answered' ==> clarificationAnswerRecord.clarificationAnswerId === elicitationTurn.clarificationAnswerId` — An answered turn must reference a stored ClarificationAnswerRecord — without this, the answer is not structurally retrievable for draft updates
- `elicitationTurn.turnId !== null && elicitationTurn.turnId.length > 0` (ElicitationTurn) — Turn must have a unique identifier — without this, session history cannot be ordered or referenced
- `elicitationTurn.sessionId !== null` (ElicitationTurn) — Turn must be bound to exactly one session — without this, turns cannot be associated with their session
- `elicitationTurn.clarificationRequestId !== null` (ElicitationTurn) — Turn must always have a ClarificationRequest — a turn without a question has no structural meaning
- `elicitationTurn.turnIndex >= 0` (ElicitationTurn) — Turn index must be non-negative — without this, temporal ordering of dialogue is undefined
- `!(elicitationTurn.status === 'answered') || elicitationTurn.clarificationAnswerId !== null` (ElicitationTurn) — An answered turn must have a ClarificationAnswer — without this, the turn is structurally incomplete and the draft cannot be updated
- `!(elicitationTurn.status === 'answered') || elicitationTurn.closedAt !== null` (ElicitationTurn) — An answered turn must have a close timestamp — without this, turn duration cannot be computed
- `adaProposal.proposalId !== null && adaProposal.proposalId.length > 0` (AdaProposal) — Proposal must have a unique identifier — without this, the session cannot track which proposals were accepted
- `adaProposal.proposedText !== null && adaProposal.proposedText.trim().length > 0` (AdaProposal) — Proposal must contain non-empty proposed text — without this, it offers no structural content to the draft
- `adaProposal.rationale !== null && adaProposal.rationale.length > 0` (AdaProposal) — Proposal must carry a rationale — without this, Ada's structural reasoning is opaque and unauditable
- `!(adaProposal.disposition === 'modified') || adaProposal.modifiedText !== null` (AdaProposal) — A modified proposal must carry the modified text — without this, the accepted version cannot be written to the draft
- `clarificationRequestRecord.clarificationRequestId !== null && clarificationRequestRecord.clarificationRequestId.length > 0` (ClarificationRequestRecord) — ClarificationRequest must have a unique identifier — without this, ElicitationTurn cannot reference the specific question asked
- `clarificationRequestRecord.unknownId !== null && clarificationRequestRecord.unknownId.length > 0` (ClarificationRequestRecord) — ClarificationRequest must reference a specific unknown — without this, the request cannot be targeted to a DraftIntentGraph field
- `clarificationRequestRecord.question !== null && clarificationRequestRecord.question.trim().length > 0` (ClarificationRequestRecord) — ClarificationRequest must contain a non-empty question — without this, the user has nothing to respond to
- `clarificationRequestRecord.impact !== null` (ClarificationRequestRecord) — ClarificationRequest must declare its impact — without this, Gap severity and elicitation priority cannot be determined
- `clarificationAnswerRecord.clarificationAnswerId !== null && clarificationAnswerRecord.clarificationAnswerId.length > 0` (ClarificationAnswerRecord) — ClarificationAnswer must have a unique identifier — without this, ElicitationTurn cannot reference the specific answer received
- `clarificationAnswerRecord.unknownId !== null` (ClarificationAnswerRecord) — ClarificationAnswer must reference the same unknown as the ClarificationRequest it answers — without this, the answer cannot be mapped to the correct DraftIntentGraph field
- `clarificationAnswerRecord.answer !== null && clarificationAnswerRecord.answer.trim().length > 0` (ClarificationAnswerRecord) — ClarificationAnswer must contain non-empty text — without this, the Gap it targets cannot be resolved and the draft cannot be updated
- `clarificationAnswerRecord.receivedAt !== null` (ClarificationAnswerRecord) — ClarificationAnswer must have a receipt timestamp — without this, turn duration and session timeline cannot be established
