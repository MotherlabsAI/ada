...
## Bounded Context
**Context:** IntentIngestion
**Entities:** Intent, TextSpan, SemanticFragment, AmbiguityMarker, ClarificationLoop, ClarificationAnswer
...
- `every SemanticFragment.sourceIntentSpan must reference a TextSpan within the bounds of its parent Intent.rawText` — fragments may not reference spans outside their intent text
- `every AmbiguityMarker.affectedFragmentId must resolve to an existing SemanticFragment within the same Intent scope` — ambiguity markers must point to real fragments
- `intent.rawText !== null && intent.rawText.trim().length > 0` (Intent) — intent must contain non-empty text
- `intent.fingerprint !== null` (Intent) — every intent must have a stable identity fingerprint
- `intent.submittedAt !== null` (Intent) — submission timestamp must be recorded
- `semanticFragment.fragmentId !== null` (SemanticFragment) — fragment must have a unique identifier
- `semanticFragment.role !== null` (SemanticFragment) — fragment must have an assigned semantic role
- `semanticFragment.sourceIntentSpan !== null` (SemanticFragment) — fragment must trace back to a span in the original intent
- `textSpan.startIndex >= 0` (TextSpan) — span start must be non-negative
- `textSpan.endIndex > textSpan.startIndex` (TextSpan) — span end must be after start
- `textSpan.text !== null && textSpan.text.length > 0` (TextSpan) — span must reference non-empty text
- `clarificationLoop.loopId !== null` (ClarificationLoop) — loop must be identified
- `clarificationLoop.sourceIntentFingerprint !== null` (ClarificationLoop) — loop must reference the originating intent
- `clarificationLoop.detectedAmbiguities !== null && clarificationLoop.detectedAmbiguities.length > 0` (ClarificationLoop) — clarification loop must have at least one detected ambiguity
- `clarificationLoop.collectedAnswers !== null` (ClarificationLoop) — answers collection must be initialized
- `ambiguityMarker.markerId !== null` (AmbiguityMarker) — marker must be identified
- `ambiguityMarker.affectedFragmentId !== null` (AmbiguityMarker) — marker must reference its fragment
- `ambiguityMarker.ambiguityType !== null` (AmbiguityMarker) — ambiguity type must be classified
- `ambiguityMarker.question !== null && ambiguityMarker.question.length > 0` (AmbiguityMarker) — ambiguity must produce a question for the user
- `clarificationAnswer.answerId !== null` (ClarificationAnswer) — answer must be identified
- `clarificationAnswer.markerRef !== null` (ClarificationAnswer) — answer must reference its ambiguity marker
- `clarificationAnswer.answerText !== null && clarificationAnswer.answerText.length > 0` (ClarificationAnswer) — answer must contain text
- `clarificationAnswer.appliedToIntentSpan !== null` (ClarificationAnswer) — answer must reference the intent span it resolves
