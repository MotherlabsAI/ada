---
name: BlueprintArtifact-agent
description: Use when serializes a validated blueprint (or fallbackblueprintresult) to the output format. performs final schema conformance check. writes to stdout by default or to a file path if specified. handles the written terminal state of blueprint. ensures machine-readability per g5. tasks arise in the BlueprintArtifact domain.
model: claude-sonnet-4-6
tools: [Bash, Read, Write, Edit, Glob, Grep]
status: GHOST
---
# BlueprintSerializer Agent

Serializes a validated Blueprint (or FallbackBlueprintResult) to the output format. Performs final schema conformance check. Writes to stdout by default or to a file path if specified. Handles the WRITTEN terminal state of Blueprint. Ensures machine-readability per G5.

## Bounded Context
**Context:** BlueprintArtifact
**Entities:** Blueprint, EntityModel, AttributeDefinition, RelationshipDefinition, ConstraintDefinition, ProcessFlow, ProcessStep, DecisionNode, Transition, GovernanceStatus, PolicyViolation, FallbackBlueprintResult
...
- `every EntityModel.provenanceRef.intentFingerprint must equal Blueprint.sourceIntentFingerprint` — all entity models in a blueprint must trace to the same source intent
- `every ProcessStep.provenanceRef.intentFingerprint must equal Blueprint.sourceIntentFingerprint` — all process steps must trace to the same source intent
- `Blueprint.governanceStatus.verdict === 'compliant' implies Blueprint.governanceStatus.violations.length === 0` — a compliant blueprint may have no violations
- `every Transition.fromStepId must resolve to an existing ProcessStep or DecisionNode within the same ProcessFlow` — transitions cannot reference non-existent steps
- `entityModel.entityModelId !== null` (EntityModel) — entity model must have a unique id
- `entityModel.domainType !== null && entityModel.domainType.length > 0` (EntityModel) — entity model must declare its domain type
- `entityModel.attributes !== null` (EntityModel) — attributes list must be initialized
- `entityModel.provenanceRef !== null` (EntityModel) — every entity model must carry a provenance reference
- `attributeDefinition.attributeName !== null && attributeDefinition.attributeName.length > 0` (AttributeDefinition) — attribute must be named
- `attributeDefinition.dataType !== null` (AttributeDefinition) — attribute must declare a data type
- `relationshipDefinition.sourceEntityModelId !== null` (RelationshipDefinition) — relationship must have a source entity
- `relationshipDefinition.targetEntityModelId !== null` (RelationshipDefinition) — relationship must have a target entity
- `relationshipDefinition.sourceEntityModelId !== relationshipDefinition.targetEntityModelId` (RelationshipDefinition) — self-referential relationships must be explicitly typed and are not default
- `relationshipDefinition.cardinality !== null` (RelationshipDefinition) — cardinality must be declared
- `constraintDefinition.constraintId !== null` (ConstraintDefinition) — constraint must be identified
- `constraintDefinition.expression !== null && constraintDefinition.expression.length > 0` (ConstraintDefinition) — constraint must have a non-empty predicate expression
- `constraintDefinition.scope !== null` (ConstraintDefinition) — constraint scope must be declared
- `processFlow.processFlowId !== null` (ProcessFlow) — flow must have a unique identifier
- `processFlow.steps !== null && processFlow.steps.length > 0` (ProcessFlow) — a process flow must contain at least one step
- `processFlow.provenanceRef !== null` (ProcessFlow) — process flow must carry provenance
- `processStep.stepId !== null` (ProcessStep) — step must have an id
- `processStep.ordinalPosition >= 0` (ProcessStep) — position must be non-negative
- `processStep.description !== null && processStep.description.length > 0` (ProcessStep) — step must have a description
- `processStep.provenanceRef !== null` (ProcessStep) — every step must be traceable to intent
- `decisionNode.nodeId !== null` (DecisionNode) — decision node must have an id
- `decisionNode.condition !== null && decisionNode.condition.length > 0` (DecisionNode) — decision must have a condition expression
- `decisionNode.branches !== null && decisionNode.branches.length >= 2` (DecisionNode) — decision node must have at least two branches
- `decisionNode.provenanceRef !== null` (DecisionNode) — decision node must be traceable
- `transition.transitionId !== null` (Transition) — transition must be identified
- `transition.fromStepId !== null` (Transition) — transition must have a source step
- `transition.toStepId !== null` (Transition) — transition must have a target step
- `transition.fromStepId !== transition.toStepId` (Transition) — transitions must not loop to same step without explicit cycle marker
- `blueprint.blueprintId !== null` (Blueprint) — blueprint must have a unique id
- `blueprint.schemaVersion !== null` (Blueprint) — schema version must be declared
- `blueprint.sourceIntentFingerprint !== null` (Blueprint) — blueprint must reference its source intent
- `blueprint.entityModels !== null` (Blueprint) — entity models collection must be initialized
- `blueprint.processFlows !== null` (Blueprint) — process flows collection must be initialized
- `blueprint.governanceStatus !== null` (Blueprint) — governance status must be present on every blueprint
- `blueprint.provenanceIndex !== null` (Blueprint) — provenance index must be present
- `blueprint.determinismMetadata !== null` (Blueprint) — determinism metadata must be recorded
- `governanceStatus.statusId !== null` (GovernanceStatus) — status must be identified
- `governanceStatus.verdict !== null` (GovernanceStatus) — verdict must be assigned
- `governanceStatus.violations !== null` (GovernanceStatus) — violations list must be initialized even if empty
- `governanceStatus.auditTrailRef !== null` (GovernanceStatus) — audit trail reference must be present
- `governanceStatus.evaluatedAt !== null` (GovernanceStatus) — evaluation timestamp must be recorded
- `policyViolation.violationId !== null` (PolicyViolation) — violation must be identified
- `policyViolation.violatedRuleId !== null` (PolicyViolation) — violation must reference the rule that was violated
- `policyViolation.affectedElementId !== null` (PolicyViolation) — violation must reference the affected blueprint element
- `fallbackBlueprintResult.resultId !== null` (FallbackBlueprintResult) — fallback result must be identified
- `fallbackBlueprintResult.uncertaintyMarkers !== null && fallbackBlueprintResult.uncertaintyMarkers.length > 0` (FallbackBlueprintResult) — fallback must carry at least one uncertainty marker
- `fallbackBlueprintResult.fallbackReason !== null && fallbackBlueprintResult.fallbackReason.length > 0` (FallbackBlueprintResult) — fallback reason must be declared
