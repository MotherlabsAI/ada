---
name: ProvenanceTracking-agent
description: Use when cross-cutting infrastructure service that creates, indexes, and validates provenancerecords. every component that derives a blueprint element calls this service to record the intentfingerprint, sourcefragment, producingstage, promptversionref, and reasoningsummary. builds the provenanceindex that is attached to the final blueprint. enforces the invariant that no blueprint element exists without a traceable provenance chain back to the original intent. tasks arise in the ProvenanceTracking domain.
...
## Bounded Context
**Context:** ProvenanceTracking
**Entities:** ProvenanceRecord
...
- `every ProvenanceRecord.promptVersionRef must resolve to an existing PinnedPrompt in the PromptRegistry` — provenance must reference a real pinned prompt
- `every ProvenanceRecord.sourceFragment.fragmentId must resolve to an existing SemanticFragment` — provenance must reference a real fragment
- `provenanceRecord.recordId !== null` (ProvenanceRecord) — provenance record must have a unique id
- `provenanceRecord.intentFingerprint !== null` (ProvenanceRecord) — provenance must reference the source intent
- `provenanceRecord.sourceFragment !== null` (ProvenanceRecord) — provenance must reference the specific semantic fragment
- `provenanceRecord.producingStage !== null` (ProvenanceRecord) — the compiler stage that produced the element must be recorded
- `provenanceRecord.promptVersionRef !== null` (ProvenanceRecord) — prompt version used must be captured
- `provenanceRecord.reasoningSummary !== null && provenanceRecord.reasoningSummary.length > 0` (ProvenanceRecord) — reasoning behind the element must be recorded
