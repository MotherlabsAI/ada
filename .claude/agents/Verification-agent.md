...
description: Use when exposes ada's verification and workflow capabilities as mcp tools for consumption by other agents. maps to @ada/mcp-server package. provides verify, workflow-spec, and agent-file-spec endpoints. tasks arise in the Verification domain.
...
Exposes ada's verification and workflow capabilities as MCP tools for consumption by other agents. Maps to @ada/mcp-server package. Provides verify, workflow-spec, and agent-file-spec endpoints.

## Bounded Context
**Context:** Verification
**Entities:** VerificationReport, SemanticDrift, CompileResult
...
- `verificationContext.report.blueprintPostcode !== null && verificationContext.report.blueprintPostcode.length > 0` — verification report must be anchored to a specific Blueprint postcode
- `verificationContext.compileResult.status === 'accepted' ? verificationContext.report.passed === true : true` — an accepted compile result must have a passing verification report
- `semanticDrift.location !== null && semanticDrift.location.trim().length > 0` (SemanticDrift) — drift must identify the file or artifact location where divergence was detected
- `semanticDrift.original !== semanticDrift.actual` (SemanticDrift) — original and actual must differ — identical values do not constitute drift
- `semanticDrift.original !== null && semanticDrift.original.trim().length > 0` (SemanticDrift) — original blueprint intent must be recorded for the drift to be traceable
- `verificationReport.entityCoverage >= 0 && verificationReport.entityCoverage <= 1` (VerificationReport) — entity coverage must be a ratio in [0,1]
- `verificationReport.invariantCoverage >= 0 && verificationReport.invariantCoverage <= 1` (VerificationReport) — invariant coverage must be a ratio in [0,1]
- `verificationReport.overallScore >= 0 && verificationReport.overallScore <= 1` (VerificationReport) — overall score must be a ratio in [0,1]
- `verificationReport.blueprintPostcode !== null && verificationReport.blueprintPostcode.length > 0` (VerificationReport) — verification report must reference the Blueprint postcode it was generated against
- `verificationReport.passed === true ? verificationReport.findings.filter(f => f.severity === 'critical').length === 0 : true` (VerificationReport) — a passing verification report must have no critical findings
- `compileResult.blueprint !== null` (CompileResult) — compile result must always carry a blueprint — even a rejected run holds the last blueprint for inspection
- `compileResult.governorDecision !== null` (CompileResult) — compile result must record the terminal governor decision
- `compileResult.iterationCount >= 1` (CompileResult) — at least one pipeline iteration must have occurred
- `compileResult.status === 'accepted' ? compileResult.governorDecision.decision === 'ACCEPT' : true` (CompileResult) — status 'accepted' may only be set when GovernorDecision is ACCEPT
- `compileResult.fallback !== null ? compileResult.status !== 'accepted' : true` (CompileResult) — a fallback result is only present when the compile did not reach ACCEPT
