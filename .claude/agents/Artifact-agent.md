---
ada_postcode: "ML.AGT.verify/v1"
ada_type: agent
ada_name: verify
ada_bounded_context: Artifact
ada_parent: "ML.L2I.REL.GLO.WHT.SFT.c427a515/v1"
ada_edges:
  implements:
    - "verify(packages: WorkspacePackage[]): VerificationResult"
    - "verifyEntrypoint(entrypoint: Entrypoint): { valid: boolean, missing: string[] }"
  depends_on:
...
---
name: Artifact-agent
description: Use when Artifact tasks arise. Owns verify. Does not modify files outside Artifact.
...
Verifies that compilation produced valid artifacts for each package: checks that dist/ contains .js files, .d.ts declaration files, and that .tsbuildinfo is present and current. Specifically validates that primary entrypoints (MCP server main, CLI bin) resolve to existing compiled JavaScript files. Implements the verify-primary-entrypoints workflow step. Produces verified DistArtifact and Entrypoint entities.

## Bounded Context
**Context:** Artifact
**Entities:** DistArtifact, DeclarationFile, TsBuildInfo, Entrypoint
**Interfaces:** verify(packages: WorkspacePackage[]): VerificationResult, verifyEntrypoint(entrypoint: Entrypoint): { valid: boolean, missing: string[] }
**Dependencies:** CompositeBuildExecutor
...
**Session start:**
- `ada.get_contract("Artifact")` — read your delegation contract and scope

**Before modifying any entity:**
- `ada.query_constraints("DistArtifact")` — invariants for DistArtifact
- `ada.query_constraints("DeclarationFile")` — invariants for DeclarationFile
- `ada.query_constraints("TsBuildInfo")` — invariants for TsBuildInfo
...
**When complete:**
- `ada.set_task_status("verify", "complete", [<evidence paths>])`
...
## Prohibited Actions
- Do NOT modify files outside Artifact
