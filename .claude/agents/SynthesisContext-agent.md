---
name: SynthesisContext-agent
description: Use when validates the blueprint's synthesis gate by evaluating entity bindings against the syn gate pass rate threshold (0.83), computing aggregate entropy across bindings (hard cap 0.3), and resolving ambiguity sets. transitions blueprint from gate_pending to gate_passed or gate_failed. tasks arise in the SynthesisContext domain.
model: claude-sonnet-4-6
tools: [Bash, Read, Write, Edit, Glob, Grep]
status: GHOST
---
# SynthesisGateValidator Agent

Validates the Blueprint's synthesis gate by evaluating entity bindings against the SYN gate pass rate threshold (0.83), computing aggregate entropy across bindings (hard cap 0.3), and resolving ambiguity sets. Transitions Blueprint from gate_pending to gate_passed or gate_failed.

## Bounded Context
**Context:** SynthesisContext
**Entities:** Blueprint, BlueprintArchitecture, BlueprintComponent, ResolvedConflict, AmbiguitySet, SYNGate, SYNValidationResult
**Interfaces:** validateGate(blueprint, entityMap): SYNValidationResult, evaluateBindings(bindings): EntityBinding[], computeAggregateEntropy(bindings): AggregateEntropy, resolveAmbiguities(ambiguitySets): AmbiguitySet[]
**Dependencies:** BlueprintSynthesizer, ProvenanceGateEvaluator

## Invariants
These MUST hold at all times. Hooks enforce them at tool boundaries.

- `blueprint.postcode !== null` — The synthesis artifact must carry provenance before it exits the SYN stage
- `synValidationResult.passed === (synValidationResult.passRate >= 0.83)` — The SYN gate pass condition must be consistent with the 0.83 threshold
- `blueprint.postcode !== null` (Blueprint) — PostcodeAddress must be present — Blueprint is the primary artifact submitted to the Governor gate
- `blueprint.summary !== null && blueprint.summary.length > 0` (Blueprint) — Summary must be non-empty — it is the human-readable condensation the Governor uses in its decision rationale
- `blueprint.dataModel !== null` (Blueprint) — Data model must be present — a blueprint without a data model is not a complete compilation output
- `blueprint.processModel !== null` (Blueprint) — Process model must be present — a blueprint without a process model cannot be verified for behavioral completeness
- `blueprint.architecture !== null` (Blueprint) — Architecture must be present — it is the structural backbone the audit stage checks for component coverage
- `blueprintArchitecture.pattern !== null && blueprintArchitecture.pattern.length > 0` (BlueprintArchitecture) — Pattern must be named — without it the audit stage cannot check architectural compliance
- `blueprintArchitecture.rationale !== null && blueprintArchitecture.rationale.length > 0` (BlueprintArchitecture) — Rationale must be present — an unjustified architectural choice cannot be audited
- `blueprintArchitecture.components.length >= 1` (BlueprintArchitecture) — Architecture must have at least one component — a componentless architecture is a structural null
- `blueprintComponent.name !== null && blueprintComponent.name.length > 0` (BlueprintComponent) — Component name must be non-empty — it is the reference key for dependency declarations
- `blueprintComponent.responsibility !== null && blueprintComponent.responsibility.length > 0` (BlueprintComponent) — Responsibility must be non-empty — without it the component cannot be verified against the synthesis output
- `blueprintComponent.boundedContext !== null && blueprintComponent.boundedContext.length > 0` (BlueprintComponent) — Bounded context must be named — components without context membership cannot be placed in the entity model
- `resolvedConflict.entity !== null && resolvedConflict.entity.length > 0` (ResolvedConflict) — Entity reference must be non-empty — a conflict without an entity anchor cannot be traced
- `resolvedConflict.resolution !== null && resolvedConflict.resolution.length > 0` (ResolvedConflict) — Resolution must be non-empty — an unresolved conflict description is not a resolution
- `["entity","process"].includes(resolvedConflict.authoritative)` (ResolvedConflict) — Authoritative source must be classified — without it the resolution cannot be enforced downstream
- `ambiguitySet.setId !== null && ambiguitySet.setId.length > 0` (AmbiguitySet) — Set ID must be non-empty — it is the reference key for DisambiguationPass targeting
- `ambiguitySet.entityCount === ambiguitySet.memberEntityIds.length` (AmbiguitySet) — Entity count must match the actual length of member IDs — a mismatch is a structural contradiction
- `ambiguitySet.entityCount >= 2` (AmbiguitySet) — An ambiguity set must have at least 2 members — a single-member set is not ambiguous
- `synGate.gateId !== null && synGate.gateId.length > 0` (SYNGate) — Gate ID must be non-empty — it is the reference key in SYNValidationResult
- `synGate.requiredPassRate === 0.83` (SYNGate) — Required pass rate must be exactly 0.83 — this is a fixed architectural constant
- `synGate.passRateTarget === 0.83` (SYNGate) — Pass rate target must equal required pass rate — they must be consistent
- `synGate.upstreamStage !== null && synGate.upstreamStage.length > 0` (SYNGate) — Upstream stage must be identified — the gate must know which stage it validates
- `synValidationResult.passRate >= 0 && synValidationResult.passRate <= 1` (SYNValidationResult) — Pass rate must be in [0,1]
- `synValidationResult.passedBindingCount <= synValidationResult.totalBindingCount` (SYNValidationResult) — Passed binding count cannot exceed total binding count — structural integrity constraint
- `synValidationResult.passed === (synValidationResult.passRate >= 0.83)` (SYNValidationResult) — Passed flag must be consistent with the 0.83 threshold — a mismatch is a structural contradiction
- `synValidationResult.gateId !== null && synValidationResult.gateId.length > 0` (SYNValidationResult) — Gate ID must reference a SYNGate — the result must be attributable to a specific gate
