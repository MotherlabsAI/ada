---
name: SYNGateEvaluation-agent
description: Use when binds the syn gate to the completed int run, evaluates the pass rate against the ≥ 0.83 target, and determines whether self-resolution was achieved or manual intervention is required. manages the syn gate state machine (unbound → evaluating → passed/failed/blocked/error). exists because syngate and gatepassrate entities model the downstream quality gate, the syn-gate-evaluation-after-int-cleanup workflow defines the evaluation sequence, and g3 sets the pass rate target. tasks arise in the SYNGateEvaluation domain.
...
## Bounded Context
**Context:** SYNGateEvaluation
**Entities:** SYNGate, GatePassRate, SYNSelfResolution
**Interfaces:** bindToIntRun(intStageRunId: RunID), evaluatePassRate(acceptedBindings: EntityBinding[]), checkSelfResolution(observedRate: number), auditTriad(entropy: AggregateEntropy, gate: GatePassRate, bindings: EntityBinding[]), getGateState()
**Dependencies:** IntegrationStageOrchestrator, EntropyEvaluator

## Invariants
These MUST hold at all times. Hooks enforce them at tool boundaries.

- `synGate.passRateTarget === gatePassRate.target` — SYN gate target and gate pass rate target are the same value
- `synSelfResolution.synGateId === synGate.id` — self-resolution is anchored to a specific SYN gate instance
- `synSelfResolution.achieved === (gatePassRate.value >= 0.83)` — self-resolution achieved iff observed pass rate meets target
- `synGate.passRateTarget === 0.83` (SYNGate) — target pass rate is invariantly 0.83
- `synGate.observedPassRate === null || (synGate.observedPassRate >= 0 && synGate.observedPassRate <= 1)` (SYNGate) — observed pass rate, when present, is a unit-interval scalar
- `synGate.intStageRunId !== null` (SYNGate) — SYN gate must reference the INT stage whose output it evaluates
- `synGate.selfResolved === true || synGate.observedPassRate === null || synGate.observedPassRate < synGate.passRateTarget` (SYNGate) — self-resolution holds only when observed pass rate meets or exceeds target
- `gatePassRate.value >= 0 && gatePassRate.value <= 1` (GatePassRate) — pass rate is a unit-interval scalar
- `gatePassRate.target === 0.83` (GatePassRate) — required target is fixed at 0.83
- `gatePassRate.meetsTarget === (gatePassRate.value >= 0.83)` (GatePassRate) — meetsTarget is derived strictly from value >= 0.83
- `synSelfResolution.requiredGatePassRate === 0.83` (SYNSelfResolution) — self-resolution threshold is fixed at 0.83
- `synSelfResolution.manualInterventionRequired === !synSelfResolution.achieved` (SYNSelfResolution) — manual intervention is required if and only if self-resolution is not achieved
- `synSelfResolution.synGateId !== null` (SYNSelfResolution) — self-resolution must be anchored to a specific SYN gate
