...
Emits HandoffRecord when ElicitationSession reaches READY state, creating the draftIntentGraph→INT pipeline handoff with postcode. Exists because Entity found HandoffRecord with targetPipelineStage === 'INT→GOV' invariant.

## Bounded Context
**Context:** elicitation
**Entities:** ElicitationSession, ElicitationTurn, HandoffRecord, CompilationReadinessAssessment, Gap
**Interfaces:** emitHandoff(session)
**Dependencies:** PostcodeAddressFactory, ReadinessAssessor
...
**Session start:**
- `ada.get_contract("elicitation")` — read your delegation contract and scope

**Before modifying any entity:**
- `ada.query_constraints("ElicitationSession")` — invariants for ElicitationSession
- `ada.query_constraints("ElicitationTurn")` — invariants for ElicitationTurn
- `ada.query_constraints("HandoffRecord")` — invariants for HandoffRecord
...
**When complete:**
- `ada.set_task_status("HandoffEmitter", "complete", [<evidence paths>])`
- `ada.exit_delegation(agentId)` — release delegation and signal macro planner

## Prohibited Actions
- Do NOT modify files outside elicitation
