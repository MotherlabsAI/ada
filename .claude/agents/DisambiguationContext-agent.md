...
description: Use when executes a second-pass disambiguation over the ambiguity set extracted from a prior run artifact. produces candidate entitybindings with computed entropy values. pure domain logic — no i/o, no side effects. does not filter by threshold (that is entropyevaluator's job per resolved conflict). tasks arise in the DisambiguationContext domain.
model: claude-sonnet-4-6
tools: [Bash, Read, Write, Edit, Glob, Grep]
status: GHOST
---
# DisambiguationEngine Agent

Executes a second-pass disambiguation over the ambiguity set extracted from a prior run artifact. Produces candidate EntityBindings with computed entropy values. Pure domain logic — no I/O, no side effects. Does NOT filter by threshold (that is EntropyEvaluator's job per resolved conflict).

## Bounded Context
**Context:** DisambiguationContext
**Entities:** AmbiguitySet, EntityBinding, DisambiguationPass
**Interfaces:** disambiguate(ambiguitySet: AmbiguitySet, passOrdinal: number): EntityBinding[]

## Invariants
These MUST hold at all times. Hooks enforce them at tool boundaries.

- `DisambiguationPass.inputAmbiguitySet.sourceRunId.value === PriorRunArtifact.runId.value` — ambiguity set fed into the pass must be hydrated from the identified prior run artifact
- `DisambiguationPass.outputBindings.length <= DisambiguationPass.inputAmbiguitySet.entries.length` — pass can only filter or resolve entries; it cannot fabricate new bindings beyond the input set
- `ambiguitySet.entries !== null` (AmbiguitySet) — entries array must exist, even when empty
- `ambiguitySet.sourceRunId !== null` (AmbiguitySet) — ambiguity set must reference the run it was hydrated from
- `ambiguitySet.entries.every(e => e.confidence !== null)` (AmbiguitySet) — every entry must carry a confidence value
- `entityBinding.sourceEntity !== null && entityBinding.sourceEntity.length > 0` (EntityBinding) — binding must identify a source entity
- `entityBinding.targetElement !== null && entityBinding.targetElement.length > 0` (EntityBinding) — binding must identify a target schema element
- `entityBinding.confidence >= 0 && entityBinding.confidence <= 1` (EntityBinding) — confidence is a unit scalar
- `entityBinding.entropy.value >= 0 && entityBinding.entropy.value <= 1` (EntityBinding) — entropy is a unit scalar
- `disambiguationPass.inputAmbiguitySet !== null` (DisambiguationPass) — pass must operate on a non-null ambiguity set
- `disambiguationPass.outputBindings.every(b => b.entropy.value < 0.30)` (DisambiguationPass) — output bindings are pre-filtered to entropy < 0.30 before leaving the pass
- `disambiguationPass.passOrdinal >= 2` (DisambiguationPass) — int-rerun always performs at least the second pass; ordinal 1 belongs to the original INT stage
