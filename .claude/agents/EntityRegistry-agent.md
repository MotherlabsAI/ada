---
name: EntityRegistry-agent
description: Use when extracts entities from the total componentpackagemapping and registers them into an entitymap via ententityregistration events. each registration targets the entitymap registry type and records a provenancerecordpostcode, entitymappostcode, and sourcecomponentid. drives the entitymap state machine through absent → draft → populated. the resulting entitycount must be > 0 for the ent gate to pass. tasks arise in the EntityRegistry domain.
model: claude-sonnet-4-6
tools: [Bash, Read, Write, Edit, Glob, Grep]
status: GHOST
---
# EntityRegistrar Agent
...
**Entities:** EntityMap, ENTEntityRegistration
...
- `context.entityMap.entityCount === context.registrations.length` — the EntityMap's count must match the number of registration records in the context
- `context.registrations.every(r => r.entityMapPostcode === context.entityMap.postcode)` — all registrations must reference the same EntityMap postcode or they belong to different maps
- `registration.targetRegistryType === "EntityMap"` (ENTEntityRegistration) — ENT-stage registrations must target EntityMap specifically; other registries are out of scope for this stage
- `registration.provenanceRecordPostcode !== null && registration.provenanceRecordPostcode.length > 0` (ENTEntityRegistration) — every registration must have provenance or the ENT gate cannot verify the three-hop chain through it
- `registration.entityMapPostcode !== null && registration.entityMapPostcode.length > 0` (ENTEntityRegistration) — the registration must reference the EntityMap it populates or it is an orphaned record
- `registration.sourceComponentId !== null && registration.sourceComponentId.length > 0` (ENTEntityRegistration) — entities must trace to a source BlueprintComponent or their origin is unverifiable
- `registration.registeredAt > 0` (ENTEntityRegistration) — registration timestamp must be a positive epoch value; zero indicates the record was never actually committed
- `entityMap.entityCount === entityMap.entities.length` (EntityMap) — the declared count must match the actual array length or the ENT gate will evaluate on wrong numbers
- `entityMap.pipelineRunId !== null && entityMap.pipelineRunId.length > 0` (EntityMap) — the EntityMap must be bound to a specific run or it cannot participate in provenance validation
- `entityMap.postcode !== null && entityMap.postcode.length > 0` (EntityMap) — the EntityMap must have a provenance address so the ENT gate can reference it as evidence of entity extraction
- `entityMap.entityCount > 0` (EntityMap) — an empty EntityMap cannot pass the ENT gate; zero entities means extraction did not occur
