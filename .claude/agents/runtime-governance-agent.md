...
Starts the stdio MCP server as a subprocess (C10). Exposes runtime governance, blueprint query, verification, and skill management tools. No standalone daemon (C10), no HTTP (C19). Exists because the mcp-server package provides this and it is the only external interface besides CLI.

## Bounded Context
**Context:** runtime-governance
**Entities:** WorldState, StateFact, DelegationContract, MacroPlan, Session, DriftEvent, RuntimeCheckpoint, AgentFile, HookScript, VerificationReport
**Interfaces:** startServer()
**Dependencies:** PipelineOrchestrator, buildWorldState, writeCheckpoint, AdaStorage
...
**Session start:**
- `ada.get_contract("runtime-governance")` — read your delegation contract and scope

**Before modifying any entity:**
- `ada.query_constraints("WorldState")` — invariants for WorldState
- `ada.query_constraints("StateFact")` — invariants for StateFact
- `ada.query_constraints("DelegationContract")` — invariants for DelegationContract
...
**When complete:**
- `ada.set_task_status("startServer", "complete", [<evidence paths>])`
- `ada.exit_delegation(agentId)` — release delegation and signal macro planner

## Prohibited Actions
- Do NOT modify files outside runtime-governance
