...
test("arrow-down moves selection and space flags the selected node", async () => {
...
test("x rejects the selected node and persists it", async () => {
  const persisted: Array<{ flagged: string[]; rejected: string[] }> = [];
  const { stdin } = render(
    h(App, {
      slug: "fixture",
  ...
  const last = persisted.at(-1);
  assert.ok(last, "expected onPersist to fire");
  assert.ok(last.rejected.includes("ATT.004"), "ATT.004 should be rejected");
});
