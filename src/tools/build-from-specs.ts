...
const { model, kept, rejected } = assemblePackGated(slug, intent, specs);
...
console.log(
  `  ${paint("quality gate", "green")}: ${bold(String(kept.length))} kept · ` +
    (rejected.length
      ? paint(`${rejected.length} rejected`, "rose") +
        dim(`  (${rejected.map((r) => r.spec.id).join(", ")})`)
      : dim("0 rejected")),
);
if (first) {
  console.log(
